<header class="main-header">
        ...
        <div class="header-container">
          ...
          <div class="authorization-block">
                @if(!$isAuth)
                <a href="{{url('/register')}}" class="authorization-block__link">Регистрация</a><a href="{{url('/login')}}" class="authorization-block__link">Войти</a>
                @else
                <a href="{{url('/home')}}" class="authorization-block__link">Личный кабинет</a>
                @endif
            </div>
        </div>
        <x-form-buy style="display:none;" :auth="Auth::user()"/>
</header>